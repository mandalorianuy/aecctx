...
def test_public_version_is_0_3_0_and_v01_schema_remains_stable() -> None:
...
def test_release_metadata_contains_checksums_and_spdx_sbom(tmp_path: Path) -> None:
    ...
    sbom = json.loads((tmp_path / "aecctx-0.3.0.spdx.json").read_text(encoding="utf-8"))
    assert artifact.name in checksums
    assert result["checksums"] == str(tmp_path / "SHA256SUMS")
    assert sbom["spdxVersion"] == "SPDX-2.3"
    assert any(package["name"] == "aecctx" and package["versionInfo"] == "0.3.0" for package in sbom["packages"])
...
def test_release_documentation_and_automation_exist() -> None:
    ...
    release_notes = (ROOT / "docs" / "releases" / "v0.3.0.md").read_text(encoding="utf-8")
    assert "pip install aecctx==0.3.0" not in release_notes
    assert "releases/download/v0.3.0/aecctx-0.3.0-py3-none-any.whl" in release_notes


def test_release_workflow_uploads_each_asset_once() -> None:
    workflow = (ROOT / ".github" / "workflows" / "release.yml").read_text(encoding="utf-8")

    assert "dist/aecctx-0.3.0*" not in workflow
    for asset in (
