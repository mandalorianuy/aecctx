...
- `NativeExtraction` serializes only canonical JSON event payloads plus `artifacts/root-<n>.brep` and `artifacts/scene.glb`.
...
- [ ] **Step 5: Implement v0.2 mapping.** Keep source entities in `evidence/primitives.jsonl`; XDE structure normalization cites observed parents; BREP/GLB records are derived with `representation_fidelity`; all provider/runtime/configuration/transfer digests and diagnostics are retained.
