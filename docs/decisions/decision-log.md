...
- Portable-gate amendment: fixture `--check` is a read-only digest/structure check with no Docker or provider call. Exact-image regeneration and byte comparison are exposed only as `--live-check`; they remain mandatory live evidence but are not prerequisites for a portable checkout. CI failure or external-runtime absence cannot be relabeled as fixture drift.
