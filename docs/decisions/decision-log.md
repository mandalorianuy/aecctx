...
- Evidence boundary: source entities and locators remain observed. XDE/kernel B-Rep is translator-derived and records transfer/healing loss; the provider emits bounded canonical triangle JSON and the core applies its existing `trimesh==4.12.2` deterministic convention to produce subordinate GLB. Names, colors, layers, units, placements and assembly/subfigure relationships retain original classes and source references. No source-exact B-Rep or consumer classification is claimed.
