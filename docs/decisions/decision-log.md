...
- Determinism decision: provider word confidence is normalized to exactly three decimal places before hashing so architecture-only floating-point noise cannot alter canonical evidence; thresholds are applied to the unrounded engine value.
...
- Root cause: `ezdxf.Drawing.saveas()` and the project-authored xref manifest writer opened output in host text mode, so Windows newline translation could make repeated DXF or `source-bundle.json` checks disagree with the LF-bound corpus. The same exact SHA passed the DXF check in another Windows job and once earlier in the failing job, proving the gate path was not byte-deterministic.
...
### ACXD-041: Closed STEP/IGES XDE, per-root and opt-in healing profile

- Date: 2026-07-14.
- Status: Accepted and completed for ACX-32 after live multi-architecture, replay, schema, adapter, package and repository gates passed.
- Runtime/API decision: retain only the reviewed operator-built `cadquery-ocp==7.9.3.1.1` binding with OCCT 7.9.3 under `oci-docker-v1` on exact Linux arm64/amd64 images. STEP uses `STEPCAFControl_Reader` and IGES uses `IGESCAFControl_Reader`; both transfer into `TDocStd_Document`. The worker reads only `XCAFDoc_DocumentTool` shape/color/layer/material tools, `TDataStd_Name`, label locations, document/source unit evidence, `BRepCheck_Analyzer`, `ShapeAnalysis_ShapeTolerance`, and the fixed `ShapeFix_Shape` profile enumerated by the normative v0.3 profile.
- Evidence decision: the v0.2 lexical source event remains immutable authority. XDE labels are separate observed translator/XDE evidence and correlate to source entities only by exact, unique retained identifiers; ambiguous or absent correlations remain explicit. Raw translator BREP, optional healed BREP and triangle/GLB artifacts cite their own parents and never replace one another.
- Recovery decision: roots transfer independently. Successful roots survive a failed sibling with `AECCTX_STEP_IGES_TRANSFER_PARTIAL`; session completeness remains partial. Invalid topology is reported per root. Healing is disabled by default; the sole enabled profile uses precision `1e-7`, minimum tolerance `1e-7`, maximum tolerance `1e-3`, calls `ShapeFix_Shape.Perform`, and emits a distinct `brep-healed-derived` artifact plus before/after validity and tolerance evidence. Healing correctness, source exactness and authoring repair remain non-claims.
- Claim decision: `step-iges.xde-structure` and `step-iges.partial-recovery` may become public `partial` only for the exact AP203/AP214/AP242 edition-1 and IGES 5.3 corpus after live arm64/amd64 equality and every mapped conformance gate passes.
- Security/license decision: all source and XDE strings remain untrusted data; external references, paths and commands are never followed. OCP Apache-2.0 and OCCT LGPL-2.1 with exception remain isolated from core; image distribution still requires the existing notice/source/relinking record. No new dependency enters wheels or sdists.
- Evidence owner: `docs/specs/step-iges-v03-profile.md`, `schemas/v0.2/step-iges-xde-event.schema.json`, `fixtures/v0.3/step-iges/`, `conformance/v0.3/step-iges-corpus.json`, `tests/test_step_iges_v03.py`, `scripts/check_step_iges_v03_conformance.py` and `docs/evidence/ACX-32.md`.
- Completion result: both bounded claims pass exact XDE metadata, per-root, raw/healed separation, six-entry replay, five-input Linux arm64/amd64 equivalence, package and canonical repository gates. ACX-33 alone is promoted; no source-exact BREP, universal healing, new schema/runtime/platform or consumer claim was admitted.

### ACXD-042: Closed bounded DWG release, xref, unit and simple-geometry profile

- Date: 2026-07-14.
- Status: Accepted and completed for ACX-33 after live multi-architecture, replay, schema, mapping, security, license, package and repository gates passed.
- Runtime/release decision: retain only GNU LibreDWG 0.13.4 API/ABI 1 under the exact ACX-24 Linux arm64/amd64 OCI registrations. Executable review selects AC1012/R13, AC1014/R14 and AC1015/R2000 because the exact image creates those outputs and upstream documents useful writing only through R2000. The image rejects its help-advertised R12 option; R2004 writing is experimental and R2007 writing is uncovered, so R12 and R2004+ remain non-claims.
- Evidence decision: direct canonicalized LibreDWG JSON is observed decoder evidence. Converted DXF and normalized simple geometry are converted/derived evidence with complete input/artifact/runtime lineage. Explicit `$INSUNITS` alone may qualify units; absent, zero, malformed, unsupported or conflicting values remain explicit. Duplicate handles remain occurrence-qualified conflicts.
- Xref decision: reuse the ACX-28 content-addressed source-bundle manifest and ceilings. Validate every declared DWG member before execution, decode each member independently, and correlate only normalized declared logical paths. The decoder never opens a retained path; ambient filesystem/network traversal, symlinks, undeclared members and cycles fail closed.
- Shared-owner amendment: ACX-28's format-neutral `source-bundle.json` contract is implemented in a shared core loader while the DXF adapter re-exports its existing public names. The schema adds only `image/vnd.dwg`; DXF behavior, limits, error codes, fixtures and claims remain regression-gated. This prevents a second security validator and does not authorize another source format.
- Packaging-gate amendment: regenerate `uv.lock` to reflect ACX-31's already governed `crs` and exact `pyproj==3.7.2` extras present in `pyproject.toml`. This repairs reproducible installation metadata only and creates no new dependency, runtime, capability or claim.
- Delivery-gate amendment: provider-emitted converted DXF files under `fixtures/v0.3/dwg/` are hash-bound binary evidence. Git text normalization is forbidden because it changes retained bytes after validation; `.gitattributes`, committed-blob comparison and clean-checkout regeneration own this invariant without changing semantic claims.
- Portable-gate amendment: fixture `--check` is a read-only digest/structure check with no Docker, provider call or site-package import and MUST run under Python `-S`. Exact-image regeneration and byte comparison are exposed only as `--live-check`; they remain mandatory live evidence but are not prerequisites for a portable checkout. CI failure or external-runtime absence cannot be relabeled as fixture drift.
- Geometry decision: the maximum 3D claim is partial for enumerated simple point/line/polyline/face/mesh evidence retained through conversion. ACIS/SAT/SAB, bodies/regions/surfaces, proxy/custom/vertical objects, exact BREP, repair and complete 3D remain unsupported or structured loss.
- Security/license decision: provider requests retain action `extract` only and an exact closed configuration set. `dxf2dwg`, `dwgwrite`, `dwgrewrite`, caller commands and writer options are unreachable. LibreDWG remains GPL-3.0-or-later outside core. Upstream issue 1037 has no fix-lineage proof, and known aggregate writer-test failures remain residual risks; the full native-decoder sandbox is mandatory.
- Claim decision: `dwg.external-provider.v03` may become public `partial` only for the exact three release profiles on both reviewed Linux architectures plus portable replay. No generic DWG, encryption, complete xref/3D, CRS, writer, other runtime/platform or consumer claim is admitted.
- Evidence owner: `docs/specs/dwg-v03-profile.md`, `schemas/v0.2/dwg-v03-event.schema.json`, `fixtures/v0.3/dwg/`, `conformance/v0.3/dwg-corpus.json`, `tests/test_dwg_v03.py`, `scripts/check_dwg_v03_conformance.py` and `docs/evidence/ACX-33.md`.
- Completion result: the exact three-version claim passed four deterministic project-authored fixtures, portable replay, live byte-equivalent Linux arm64/amd64 execution, closed bundle/xref mapping, unit and simple-geometry ceilings, writer and adversarial denial, wheel/sdist GPL-runtime exclusion and the canonical repository gate. ACX-34 alone is promoted; no generic DWG, R12/R2004+, encryption, complete xref/3D, CRS or consumer claim was admitted.

### ACXD-043: Renewed no-provider RVT boundary

- Date: 2026-07-14.
- Status: Accepted for ACX-34 blocked-boundary implementation; it authorizes no decoder, provider, fixture or semantic claim.
- Human-selection decision: neither `licensed-local-runtime` nor `aps-network-provider` has been explicitly selected and authorized. Product or API availability is not authorization.
- Local-route evidence: no written Revit or ODA BimRv automation/redistribution entitlement, exact runtime and RVT version pair, complete admitted local enforcement profile, private provider CI, legally publishable real RVT fixture rights, or reviewed lifecycle/security support was supplied. ACX-25 remains an executable unsupported native-enforcement outcome.
- Remote-route evidence: no Autodesk APS application credentials, offer/billing authorization, upload consent, retention/deletion policy, allowed region/jurisdiction, telemetry policy, exact engine/app-bundle/activity versions, fixture rights or live provider CI was supplied. ACX-26 proves the neutral remote protocol only and cannot establish Autodesk service entitlement, behavior or deletion.
- Source review: Autodesk documents the desktop API as installed with Revit and APS Automation as an account-backed CAD-engine service with paid usage; ODA continues to present BimRv as a separately licensed extension. These facts establish technical routes, not the missing authorization or evidence.
...
- Delivery-gate amendment: exact-head CI exposed a non-portable test-only assumption that `.venv/bin/python` exists inside every checkout. ACX-35 subprocess conformance checks use `sys.executable`, binding child replay to the active canonical test runtime without changing product behavior, dependencies, fixtures or claims.

### ACXD-045: Closed expanded bounded IDS 1.0 quality-gate profile

- Date: 2026-07-14.
- Status: Accepted and completed for the exact public partial ACX-36 profile.
- Standard/runtime decision: retain buildingSMART IDS `v1.0.0` final commit `1effec6f419798ce09617416d258a35bdc58320a`, namespace `http://standards.buildingsmart.org/IDS`, optional `ifctester==0.8.5` and `ifcopenshell==0.8.5`, IFC2X3/IFC4 and the existing fixed shell-free local worker. The dependencies remain LGPL-3.0-or-later optional extras outside core.
- Profile decision: select only `aecctx-gate-v1-ids-1.0-expanded-v1`. It adds `partOf` for exact relations `IFCRELAGGREGATES`, `IFCRELASSIGNSTOGROUP`, `IFCRELCONTAINEDINSPATIALSTRUCTURE` and `IFCRELNESTS`, with exact nested entity name and optional predefined type. It adds XSD string `pattern` and `enumeration`, numeric `minInclusive`, `maxInclusive`, `minExclusive` and `maxExclusive`, plus specification cardinality required (`1..unbounded`), optional (`0..unbounded`) and prohibited (`0..0`). Existing simple-value facet cardinality semantics remain unchanged.
- Compatibility decision: add only optional `ids_profile` to `ids.specification` configuration. Absence or `aecctx-gate-v1-ids-1.0-simple-v1` preserves the exact v0.2 preflight and result contract; `aecctx-gate-v1-ids-1.0-expanded-v1` alone enables ACX-36. Unknown selectors fail closed. No existing policy or corpus byte changes.
- Authority decision: results remain IDS information-requirement observations over explicit IDS/IFC bytes. They do not establish source geometry, quantities, engineering approval, regulatory conformance, construction readiness or consumer acceptance. JSON remains authority; Markdown and CI JSONL remain deterministic projections.
- Safety decision: preflight admits only the exact facets, relations, restriction operators/base types and cardinalities above; every other IDS construct produces a stable unsupported finding before worker launch. XML active content, malformed input, path/digest drift, dependency mismatch, timeout/crash/output overflow and remote access remain fail-closed. Pattern execution stays inside the existing bounded worker timeout/process boundary.
- Completion result: 23 unchanged official pairs plus 22 independent project cases pass under the exact runtime and profile. v0.2 compatibility, unsupported/security outcomes, deterministic regeneration/projections, clean package boundaries and repository gates pass. Only `quality-gate.ids-expanded` is public `partial`; every stated non-claim remains unchanged.
- Fixture/license decision: selected buildingSMART cases are copied byte-for-byte from the fixed commit under CC BY-ND 4.0 with hashes and attribution. Apache-2.0 project cases remain separate and cover every positive/negative combination plus unsupported and hostile inputs.
- Claim decision: `quality-gate.ids-expanded` may become public `partial` only for the exact profile after official/project corpus, v0.2 compatibility, missing-extra, no-network, worker-limit, JSON/Markdown/CI parity, packaging and exact-SHA Linux/macOS/Windows gates pass. URI/bSDD, remote lookup, geometry, quantity-specific semantics, unlisted relations/facets/restrictions/cardinalities/schemas and approval remain non-claims.
- Evidence owner: `docs/specs/quality-gate-v03-profile.md`, `fixtures/v0.3/gate/`, `conformance/v0.3/gate-corpus.json`, `tests/test_gate_v03.py`, `scripts/check_gate_v03_conformance.py` and `docs/evidence/ACX-36.md`.

### ACXD-046: Closed local inspector distribution and portability profile

- Date: 2026-07-14.
- Status: Accepted for ACX-37 implementation; it creates no public claim until lifecycle, parity, security, package and repository gates pass.
- Host decision: select only `codex-local-plugin-contract-v1` on Python 3.12 Linux, macOS and Windows with AECCTX `>=0.2.0,<0.4.0` and exact locked MCP `1.28.1`. Broader MCP metadata remains compatibility guidance, not a portability claim. Hosted Codex, product-build compatibility, workspace rollout, model behavior and third-party hosts remain unclaimed.
- Distribution decision: select deterministic ZIP profile `aecctx-inspector-distribution-v1` with a single top-level directory, fixed metadata, canonical embedded file inventory and mandatory archive SHA-256. Install is create-only; upgrade is strictly increasing and rollback-safe; uninstall requires exact inventory.
- Signature decision: checksum and inventory are mandatory integrity. Optional detached Ed25519 signs only the canonical archive statement with an explicit caller-supplied raw public key. Absence is `not_provided`; it cannot imply publisher identity, trust, authorization, revocation, timestamp or marketplace authenticity. `cryptography>=45,<50` remains optional.
- Surface decision: retain exactly validate/info/query/diff/context/gate through fixed local stdio MCP. No shell, arbitrary command, provider execution, mutation, trust-root selection, waiver creation, network action or new semantics is admitted.
- License/security decision: plugin and project fixtures remain Apache-2.0. Archives and results are bounded untrusted data and are never executed during install. Core has no plugin dependency and plugin distributions contain no native, GPL or commercial decoder.
- Marketplace decision: the current official workspace plugin surface requires separate administrator/registry enablement. This repository has no marketplace authorization or registry publication evidence, so marketplace publication is an explicit non-claim.
- Claim decision: `codex.aecctx-inspector-distribution` may become public `partial` only for the exact three-host contract matrix after ACX-37 conformance and exact-head CI pass.
- Evidence owner: `docs/specs/inspector-distribution-v03-profile.md`, `fixtures/v0.3/plugin/`, `conformance/v0.3/plugin-corpus.json`, `tests/test_codex_plugin_v03.py`, `scripts/build_inspector_distribution.py`, `scripts/check_inspector_v03_conformance.py` and `docs/evidence/ACX-37.md`.
- Completion result: the exact three-host contract matrix passes deterministic build, mandatory checksum/inventory, optional signature mutation, verified-byte install, upgrade/rollback/downgrade/uninstall, six-operation parity, prompt-injection, provider-shell denial, clean package and repository gates. Only `codex.aecctx-inspector-distribution` is public `partial`; every marketplace, host-product, trust, model and provider non-claim remains unchanged.
