...
- Final exact-head CI also exposed a pre-existing ACX-28 gate defect: host text-mode newline translation plus process replacement made repeated DXF fixture regeneration nondeterministic and its drift detail opaque on Windows. ACXD-038 corrects the shared owner with canonical in-memory serialization, explicit hash-seed propagation, preserved diagnostics and an emulated Windows-newline regression; no DXF fixture or entry hash changed.
