...
- `./scripts/verify.sh` passed its portable subset (253 tests), full suite (700 passed, 10 intentional skips), deterministic corpora, wheel/sdist and clean release checks, plus healthy `baseline-shared-v1` integration with zero issues. Exact-SHA GitHub CI and squash merge remain delivery acceptance evidence.
...
- Final exact-head CI also exposed a pre-existing ACX-28 gate defect: host text-mode newline translation in DXF/manifest writes plus process replacement made repeated fixture regeneration nondeterministic and its drift detail opaque on Windows. ACXD-038 corrects the shared owner with canonical byte serialization, explicit hash-seed propagation, preserved diagnostics and an emulated Windows-newline regression; no DXF fixture or entry hash changed.
