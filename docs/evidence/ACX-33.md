...
Exact-head GitHub run `29356222503` then proved that the initial fixture `--check` was not portable: Ubuntu and macOS attempted unavailable Docker/image inspection, while Windows terminated in that external generation path. Follow-up runs `29356859379` and `29356861708` showed Windows still terminated while importing the provider aggregate before the read-only branch. The governed correction makes `--check` read-only, provider-free and runnable under Python `-S`; full exact-image regeneration remains under `--live-check`. Neither portable replay nor this correction replaces the already executed live arm64/amd64 matrix.

Final local closure produced 255 tests in the full-dependency phase and 743 passed with 13 intentional provider/platform skips in the portable matrix. Wheel and sdist built successfully; both passed the corrected GPL-runtime scan, release verification and baseline integration. Commit, PR, exact-head CI and squash merge remain delivery evidence recorded by the task closeout.
