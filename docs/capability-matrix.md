...
| Optional remote provider protocol | `target`: `remote-https-spki-v1` implementation and repository-owned TLS/replay corpus exist, but the claim is not accepted or public until canonical remote CI and required OneDev review/delivery complete | Third-party service availability, provider-side sandboxing/deletion, billing, jurisdiction, entitlement, semantic correctness, Web PKI lifecycle, discovery, mTLS and OAuth remain unsupported | ACX-26 `in_progress`; `sandbox.remote-provider` target |
