...
| AEC delivery quality gate | `unsupported` | Implementation candidate includes strict policy/evaluator/CLI, deterministic projections and a 27-case hash-bound offline corpus for the proposed `aecctx-gate-v1-ids-1.0-simple-v1` profile; the registry remains `target` and no public support is claimed before Task 9 acceptance | ACX-21 in progress; Tasks 1-8 implementation candidate |
