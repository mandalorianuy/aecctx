...
| Optional remote provider protocol | `target`: `remote-https-spki-v1` implementation and repository-owned TLS/replay corpus exist, but the claim is not accepted or public until the governed GitHub PR/check/squash delivery completes | Third-party service availability, provider-side sandboxing/deletion, billing, jurisdiction, entitlement, semantic correctness, Web PKI lifecycle, discovery, mTLS and OAuth remain unsupported | ACX-26 `in_progress`; `sandbox.remote-provider` target |
