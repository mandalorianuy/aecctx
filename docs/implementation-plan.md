...
ACX-29 delivery amendment: exact-head Windows CI exposed host-newline drift in the ACX-28 DXF fixture generator when the canonical gate regenerated ASCII DXF more than once. ACXD-038 governs a shared-gate correction limited to canonical in-memory DXF serialization plus a Windows-newline regression test; DXF product semantics, fixtures, hashes and claims may not change. ACX-29 remained `in_progress`, ACX-30 remained `pending`, and no delivery may merge until the complete exact-head check set is green.
