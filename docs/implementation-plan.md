...
| ACX-21 | in_progress | Tasks 1-7 contracts/parser/waivers/checks/baseline/IDS/CLI/projections complete; Task 8 conformance corpus is pending-next |
...
Governed execution rule: Tasks 1-7 completed the closed schemas/models, strict deterministic policy parser, canonical finding identity, aggregate outcome/exit precedence, exact-finding waiver lifecycle, authoritative package checks, all governed semantic baseline-diff categories, bounded IDS v1.0 simple-value evaluation and the deterministic CLI/canonical/derived projection surface with test-first evidence. Task 7 adds no corpus or capability claim. Task 8 in the subordinate plan is the sole `pending-next` task. ACX-21 remains `in_progress`, ACX-22 remains `pending` and the capability remains public `unsupported`.
