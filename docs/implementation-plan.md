...
ACX-26 delivery blocker: implementation, focused/full/canonical local gates, packaging and exact-SHA GitHub CI on Linux/macOS/Windows are green, but this checkout exposes no OneDev URL/project/credential/delivery contract or integration. ACX-26 therefore remains `in_progress`, its claim remains `target`, and ACX-27 remains `pending` until the required OneDev review and squash merge are verifiably complete.
