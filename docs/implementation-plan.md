...
ACX-26 delivery blocker: implementation, local focused tests, full pytest and packaging checks are present, but this checkout exposes no OneDev URL/project/credential/delivery contract or integration. ACX-26 therefore remains `in_progress`, its claim remains `target`, and ACX-27 remains `pending` until canonical/exact-SHA evidence plus the required OneDev review and squash merge are verifiably complete.
