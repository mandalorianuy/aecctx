...
| Signature-wrapper substitution or confusion | exact detached JWS General JSON shape, exact protected `typ` and statement SHA-256, no payload member, no unprotected/unknown headers | malformed envelope, wrong `typ`, foreign-statement hash, attached payload and unknown-header rejection |
...
| Expired, not-yet-valid, rotated, revoked or compromised key accepted | explicit policy time, independent key lifecycle/trust states, key validity interval, revocation state/time, distinct key IDs and threshold evaluation | before/after validity and revocation, untrusted expired key, rotation and multi-signature corpus |
| Host-clock nondeterminism | policy supplies `verification_time`; host clock is never consulted for trust; without policy the key/trust/authorization axes remain `not_evaluated` | repeated verification at fixed time, no-policy result and clock monkeypatch test |
